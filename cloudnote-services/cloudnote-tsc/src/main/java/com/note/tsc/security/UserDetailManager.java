...
public class UserDetailManager implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (redisServer.hasKey(username + RedisUtils.ROLE_SUF)) {
            redisServer.deleteObject(username.toUpperCase() + RedisUtils.ROLE_SUF);
        }
        if (redisServer.hasKey(username + RedisUtils.PERMISSION_SUF)) {
            redisServer.deleteObject(username.toUpperCase() + RedisUtils.PERMISSION_SUF);
        }

        redisServer.setList(username.toUpperCase() + RedisUtils.ROLE_SUF, roleList);
        redisServer.setList(username.toUpperCase() + RedisUtils.PERMISSION_SUF, permissions);
